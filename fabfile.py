...
required_settings = ['host_string', 'path', 'repo_url', 'supervisor_conf_dir']
...
def symlink_nginx():
    ...
    with cd(env.path):
        sudo('ln -sf $PWD/server/nginx/{host}.conf '
            '{nginx_conf_dir}/dataexpress-site-{host}.conf'.format(**env))


@host_context
def symlink_apache():
    "Symlinks the apache config to the host's apache conf directory."
    with cd(env.path):
        sudo('ln -sf $PWD/server/apache/{host}.conf '
            '{apache_conf_dir}/dataexpress-site-{host}.conf'.format(**env))
...
@host_context
def reload_apache():
    "Reloads apache if the config test succeeds."
    symlink_apache()

    if sudo('/etc/init.d/httpd configtest').succeeded:
        sudo('/etc/init.d/httpd reload')
    elif not confirm(yellow('apache config test failed. continue?')):
        abort('apache config test failed. Aborting')
...
def reload_supervisor():
    ...
    with cd(env.path):
        sudo('ln -sf $PWD/server/supervisor/{host}.ini '
            '{supervisor_conf_dir}/dataexpress-site-{host}.ini'.format(**env))
...
def reload_wsgi():
    ...
    pid = run('supervisorctl pid dataexpress-site-{host}'.format(host=env.host))
    try:
        int(pid)
